...
- **Market Phase Detection**：根据活跃地址与市值判断牛市（bull）、熊市（bear）或震荡（range）阶段，可在 `utils/config.yaml` 的 `market_phase.symbols` 列表（或单个 `symbol`）指定参与判断的交易对，默认仅使用 BTCUSDT。函数在传入多个币种时会返回各币种的阶段及相对强度 `S`，例如：

```python
{
    "BTCUSDT": {"phase": "bull", "S": 1.8},
    "ETHUSDT": {"phase": "bear", "S": 0.7}
}
```
