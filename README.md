...
- **Market Phase Detection**：根据活跃地址与市值判断牛市、熊市或震荡阶段，可在 `utils/config.yaml` 的 `market_phase.symbols` 列表（或单个 `symbol`）指定参与判断的交易对，并可通过 `window`、`metrics` 与 `weights` 调整计算方式，默认仅使用 BTCUSDT。
