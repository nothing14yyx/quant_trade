...
def test_community_metrics(monkeypatch):
                    ...
                    "data": [
                        {
                            ...
                            "metrics": [
                                {
                                    "metric": "AdrActCnt",
                                    "frequencies": [{"community": True}],
                                },
                                {
                                    "metric": "FeesUSD",
                                    "frequencies": [{"community": False}],
                                },
                                {
                                    "metric": "SplyCur",
                                    "frequencies": [{"community": True}],
    ...
    metrics = loader.community_metrics("btc")

    assert metrics == ["AdrActCnt", "SplyCur"]


def test_update_cm_metrics_filter(monkeypatch):
    engine = sqlalchemy.create_engine("sqlite:///:memory:")
    with engine.begin() as conn:
        conn.exec_driver_sql(
            "CREATE TABLE cm_onchain_metrics (symbol TEXT, timestamp TEXT, metric TEXT, value REAL, PRIMARY KEY(symbol, timestamp, metric))"
    ...
    monkeypatch.setattr(
        loader, "community_metrics", lambda asset: ["AdrActCnt", "SplyCur"]
    )
    ...
    loader.update_cm_metrics(["BTCUSDT"], filter_community=True)
